package com.linuxgods.kreiger.util.org.apache.commons.text.similarity;

/*
 * Copied and adapted from Apache commons-text
...
public class LevenshteinDistance {
     ...
     */
    public Integer apply(final List left, final List right) {
        if (threshold != null) {
     ...
     * @return result distance, or -1
     */
    public static int limitedCompare(List left, List right, final int threshold) { // NOPMD
        ...
        // iterates through t
        for (int j = 1; j <= m; j++) {
            final Object rightJ = right.get(j - 1); // jth character of right
            d[0] = j;
     ...
     * @throws IllegalArgumentException if either String input {@code null}
     */
    private static int unlimitedCompare(List left, List right) {
        if (left == null || right == null) {
            throw new IllegalArgumentException("Strings must not be null");
        }
